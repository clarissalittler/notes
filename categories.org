...
* Ends and Coends
  ...
  ^-- so wikipedia gives the definition of ends and coends in terms of extranatural transformations,
  which isn't actually a definition I'd seen before. I'm much more use to the Mac Lane style of defining them in terms of dinatural transformations, where dinatural transformations are an analog of natural transformations for a bi-functor (one covariant one contravariant argument).

  Ends and coends are basically (co)limit-like things for dinatural transformations.  
